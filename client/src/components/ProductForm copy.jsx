...
const ProductForm = ({
  ...
  const handleSelectImage = (files) => {
    ...
    if (files?.length > 0) {
      ...
      let seledtedFiles = [...files]
      let isChecked = true

      seledtedFiles.forEach((file) => {
  ...
  const handleDeleteFile = async (image) => {
    try {
      const res = await axios.delete(`/api/files/delete/id/${_id}/${image}`)
      if (res.data.success) {
        setImages((prev) => prev.filter((data) => data !== image))
      }
    } catch (error) {
      console.log(error)
    }
  ...
  function handleUpdateImageOrder(images) {
    setImages(images)
  }

  const handleUploadSumit = async (files) => {
  ...
  const handleSubmit = async (e) => {
    ...
    //formData.append('category', otherData.category)
    ...
    if (_id) {
      try {
        ...
        if (res.data.success) {
          alert('Product edit succeffully')
          setOtherData({
            name: existingTitle || '',
            description: existingDescription || '',
            price: existingPrice || '',
      ...
      try {
        ...
        if (res.data.success) {
          alert('Product save succeffully')
          setOtherData({
            name: existingTitle || '',
            description: existingDescription || '',
            price: existingPrice || '',
  ...
  return (
    <div>
      ...
      <ReactSortable
        className="mb-2 py-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-2"
        list={images}
      ...
      <button
        ...
        className={`btn-primary ${
          !isFormValid && 'bg-gray-300 text-gray-700 cursor-not-allowed'
        }`}
      >
        Save
      </button>
